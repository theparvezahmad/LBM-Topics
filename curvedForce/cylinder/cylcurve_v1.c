...
const int dispFreq	= 	 10;
const double D			= 	20.0;
const double tau		=		0.62;
const double rho0		= 	 1.0;
const double uInlet	=	 0.04;
				...
				if (isn[i][j] == 0)
				{
					for (a=0; a<q; a++)
					{
						...
						if (isn[ia][ja]==1) //Cylinder
						{
							...
							fx[1] += ex[a]*2.0*(-ft[kb[a]][ia][ja] + ft[a][i][j]);//*(1.0-isn[i][j]);
							fy[1] += ey[a]*2.0*(-ft[kb[a]][ia][ja] + ft[a][i][j]);//*(1.0-isn[i][j]);
